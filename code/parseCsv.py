...
work_col_name = ['id', 'start_t', 'end_t', 'country_wide_pos', 'ccp_pos', 'province_code', 'pos_ind','level']


def produce_id_name_mapping(fileIn="./csv/data_bio.csv", fileOut="./parseCsv/id_name.csv"):
...
def produce_id_work_mapping(fileIn="./csv/data_work.csv", fileOut="./parseCsv/id_work.csv"):
	...
	# Variables
	id_work_mapping = {}
	work_orig_col_ind = [0, 3, 4, 5, 6, 7, 9, 15, 21]
	...
	with open(fileIn, 'rU') as csvfile:
		...
		for row in reader:
			...
			std_data = "1/1/1900"
			date_format = "%m/%d/%Y"
			std_f = datetime.strptime(std_data, date_format)
			st_f = datetime.strptime(st_s, date_format)
			et_f = datetime.strptime(et_s, date_format)
			st_d = st_f - std_f
			et_d = et_f - std_f
			# print st_d.days, et_d.days

			temp_row = []
			temp_row.append(int(row[0]))
			temp_row.append(st_d.days)
			temp_row.append(et_d.days)
			...
			if row[9] == '':
				temp_row.append('000000')
			else:
				temp_row.append(row[9])
			temp_row.append(row[15])
